...
class HTTPOk:
    ...
    def __init__(self, rpc, programs, any, url, timeout, statuses, inbody,
                 email, sendmail, coredir, gcore, eager, retry_time, name, allowed_retries=0):
        self.rpc = rpc
...
def main(argv=sys.argv):
    short_args="hp:at:r:c:b:s:m:g:d:eEn:"
    long_args=[
        "help",
        "program=",
        "any",
    ...
    prog = HTTPOk(rpc, programs, any, url, timeout, statuses, inbody, email,
                  sendmail, coredir, gcore, eager, retry_time, name, allowed_retries)
    prog.runforever()
